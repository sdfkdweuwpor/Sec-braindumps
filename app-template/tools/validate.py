...
def main():
    ...
    if not qs:
        # Nothing extracted is a failure, not a pass: an empty bank would
        # otherwise sail through every per-question check.
        print("FAIL  data/questions.json has no questions -- put one PDF in "
              "source/ and run tools/extract.py first")
        sys.exit(1)
