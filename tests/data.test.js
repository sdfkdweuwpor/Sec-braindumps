...
test('the bank is non-trivial and every id is unique', () => {
  assert.ok(ALL_QUESTIONS.length > 900, `only ${ALL_QUESTIONS.length} questions`);
...
test('no answer letter dominates, which would mean a misaligned key', () => {
  const counts = {};
  let total = 0;
...
test('domain files partition the bank with no overlap', () => {
  const perDomain = {};
  ...
  for (const d of [1, 2, 3, 4, 5]) assert.ok(perDomain[d] > 50, `domain ${d} only has ${perDomain[d]}`);
});

test('explanation provenance is consistent', () => {
  for (const q of ALL_QUESTIONS) {
